...
import getProductIdsByCart from "../../orders/domain/use-cases/get-product-ids-by-cart";
import StocksStorage from "../storage/stock.storage";
import OrderEntity from "../../orders/storage/order.entity";
import updateStocksByOrder from "../domain/use-cases/update-stocks-by-order";
...
export default class StocksService {
  ...
  async update(order: OrderEntity, oldOrder?: OrderEntity): Promise<StockEntity[]> {
    const stockEntities = await this.stocksStorage.find({
      productIds: getProductIdsByCart([...order.cart, ...(oldOrder?.cart ?? [])]),
    });

    return this.stocksStorage.saveMany(updateStocksByOrder(order, stockEntities, oldOrder));
  }
}
